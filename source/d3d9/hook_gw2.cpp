...
int hook_gw2::vs_count = 0; //Used to save ref of second Us pattern and not first (second is 241th vs created)
...
void hook_gw2::PresentHook(Direct3DSwapChain9* _implicit_swapchain) {
...
HRESULT hook_gw2::CreateVertexHook(const DWORD *pFunction, IDirect3DVertexShader9 **ppShader, IDirect3DDevice9* _orig, Direct3DSwapChain9* _implicit_swapchain) {
	...
	if (_pShaderChS == NULL || _pShaderUs == NULL || _pShaderSt == NULL) {
		...
		if (_pShaderSt == NULL && checkPattern(pFunction, l, patternStable, 13)) {
			...
			_pShaderSt = *ppShader;
			return hr;
		} else if (_pShaderUs == NULL && checkPattern(pFunction, l, patternUnstable, 24) && vs_count > 150) {
...
HRESULT hook_gw2::SetVertexHook(IDirect3DVertexShader9 *pShader, IDirect3DDevice9* _orig, Direct3DSwapChain9* _implicit_swapchain) {
	...
	if (isInjectionShaderSt(pShader)) {
		if (!fx_applied && !can_use_unstable) {
			fx_applied = true;
